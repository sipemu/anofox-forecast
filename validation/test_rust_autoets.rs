use forecast::core::TimeSeries;
use forecast::models::exponential::auto_ets::{AutoETS, AutoETSConfig};
use forecast::models::exponential::ets::{ETSSpec, ErrorType, SeasonalType, TrendType, ETS};
use forecast::models::Forecaster;
...
fn main() {
    ...
    let file = File::open("data/noisy_seasonal.csv").expect("Failed to open file");
    ...
    println!("Mean: {:.2}, Std: {:.2}",
        ...
        (values.iter().map(|x| x.powi(2)).sum::<f64>() / values.len() as f64 -
         (values.iter().sum::<f64>() / values.len() as f64).powi(2)).sqrt()
    );
    ...
    let models_to_test = vec![
        ("A,N,N (SES)", ETSSpec::new(ErrorType::Additive, TrendType::None, SeasonalType::None), 1),
        ("A,A,N (Holt)", ETSSpec::new(ErrorType::Additive, TrendType::Additive, SeasonalType::None), 1),
        ("A,N,A (Seasonal)", ETSSpec::new(ErrorType::Additive, TrendType::None, SeasonalType::Additive), 12),
        ("A,A,A (HW Add)", ETSSpec::new(ErrorType::Additive, TrendType::Additive, SeasonalType::Additive), 12),
    ];
    ...
    if let Some(spec) = auto_model.selected_spec() {
        ...
        let forecasts = forecast.point_forecasts();
        println!("\nForecasts:");
